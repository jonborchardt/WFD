...
- [ ] **Rate limiter** — self-throttling client used by every YouTube call. Build this *before* any fetcher so nothing can bypass it.
...
- [ ] **YouTube transcript fetcher** — pull transcripts for a given video id, write to `data/transcripts/`.
...
- [ ] **Video ↔ transcript catalog** — durable catalog mapping each video to its transcript file (and back).
...
- [ ] **Gap detection** — identify videos with no transcript and queue them for another fetch attempt or a user action.
...
- [ ] **Fetch-tool tests** — vitest coverage for the YouTube fetcher and rate limiter.
...
- [ ] **Navigation UI** — browse the catalog of videos and transcripts.
...
- [ ] **UI tests** — vitest coverage for the navigation UI.
...
- [ ] **NLP entity extraction** — pull people, things, times, events, locations, organizations from transcripts.
...
- [ ] **NLP relationship extraction** — extract relationships between entities; every relationship MUST carry a transcript-span evidence pointer.
...
- [ ] **Graph store** — persist entities, relationships, and evidence pointers; expose basic queries.
...
- [ ] **AI enrichment pass** — Claude-Code-driven batch pass that refines existing relationships and surfaces ones NLP missed.
...
- [ ] **Relationship map visualization** — graph view of entities and edges.
...
- [ ] **Query** — look up an item and see its relationships and evidence.
...
- [ ] **Per-claim truthiness** — extract end-of-transcript true/false judgments and attach truthiness to the relevant relationships.
...
- [ ] **Derived truthiness propagation** — propagate truth across the graph.
...
- [ ] **Contradiction & loop detection** — surface conflicting truths, conflicting falsehoods, and logical loops.
...
- [ ] **Novel-relationship detection** — surface implicit links nobody stated outright.
...
- [ ] **Real-time skeptic score** — infer from a transcript whether a speaker is acting evasively / lying.
...
- [ ] **Public read-only website** — anyone can browse the graph and follow evidence back to source video.
