...
export const ELK_LAYOUT_CONFIGS: Record<string, Record<string, string> | null> = {
  stress: {
...
export function circularLayout(allNodes: GraphNode[]): Positions {
...
export function radialLayout(
  ...
  for (const [lv, ids] of rings) {
    const radius = lv === 0 ? (ids.length === 1 ? 0 : 80) : lv * ringSpacing;
